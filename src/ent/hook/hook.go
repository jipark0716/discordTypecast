...
// Mutate calls f(ctx, m).
func (f UserTypecastSettingFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.UserTypecastSettingMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserTypecastSettingMutation", m)
	}
	return f(ctx, mv)
}
